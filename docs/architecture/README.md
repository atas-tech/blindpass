...
| `blindpass-controller` | axum HTTP API, sqlx store (SQLite WAL or PostgreSQL, schema version 9) and a local administration Unix socket. Production startup requires the 32-byte issuer seed in `BLINDPASS_ISSUER_KEY_FILE`. Subcommands: `serve`, `check-config`, `migrate`, `reconcile-clock` and test-mode `seed --fixture` |
