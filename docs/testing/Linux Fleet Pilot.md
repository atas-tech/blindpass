...
| ID | Scenario | Required outcome |
|---|---|---|
| C01 | Exercise both direct pre-exec helper delivery and stock system-scope `LoadCredential=` through a root-owned mode-0600 broker socket. | Both routes verify root UID, obtain `SO_PEERPIDFD`, and resolve unit and invocation atomically. Direct helper: match the framed unit claim to that identity. Native `LoadCredential=`: match the untrusted abstract route's unit to the pidfd-derived unit, require a present invocation and exact administrator mapping. Both deny user-manager peers and close without error bytes on denial. |
