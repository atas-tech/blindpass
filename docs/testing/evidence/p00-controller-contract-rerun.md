...
On 2026-09-24 the user accepted the corrected 12-plus-2 Rust route scope and CT12/P02-D11 configured-issuer admin authority, and recorded CT15 as P02-D12. The earlier open-decision statements above record the state at the time of those runs. A 2026-09-25 source check found that P02-D12 names per-agent request/exchange windows that neither server implements; CT15 still exercises only the token-mint IP limit and reset, so the record needs reconciliation. Hosted PR-CI evidence remains open. Current P02 results are in the [P02 evidence](p02-controller-api-migration-rerun.md).
