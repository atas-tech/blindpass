...
The latest P03 VM evidence is recorded in [the P03 execution record](evidence/p03-fleet-authorization-execution.md). It covers broker-applied key rotation, protocol-mismatch fail-stop and recovery, phase-local E01/E02, partition/restart/replay, bounded reconnect recovery without systemd restarts, rejection of a captured signed time reply after broker restart, denial of a still-live unconsumed grant after a full guest reboot, grant expiry across real guest suspend and wall-clock rollback, and a delayed expired-grant rejection audited exactly once on both backends. Focused broker control-socket tests cover stale policy and grant/node revocation replay across broker restart; they do not exercise those delays and replays through the node HTTPS transport. Controller clock rollback, other reboot cases, full transport-level policy/revocation replay, and the broader pilot matrix remain open.
