...
- **CT15 (P02-D12):** Self-hosted SPS limits token mints per client IP; hosted SPS additionally applies workspace burst and daily quotas. The Rust controller retains the per-IP mint limit and adds independent per-agent/per-tenant request and exchange windows (default 60 calls each per 60 seconds) under P02 follow-up WI-2. Rust CT15 covers those windows, provider and tenant separation, denied-attempt accounting, 429 privacy and reset. Hosted paid-tier quota and billing behavior remain excluded.
...
P00 exit remains open until the revised evidence runs on a committed SHA with hosted PR CI. CT12 and CT14 were accepted on 2026-09-24 and recorded in the paired P02 plans.
