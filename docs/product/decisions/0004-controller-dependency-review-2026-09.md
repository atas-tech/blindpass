...
4. Keep the P02 port behind its HTTP contract gate. This dependency approval does not resolve CT12, CT14, CT15, or the CT19 browser-status decision.
...
- First dependency-slice check: `cargo check --workspace` passed with Rust 1.98.1. The locked dependency graph contains 255 packages. Formatting check passed; the prescribed locked build/lint gates remain to run.
...
- CI inspection: [Rust workflow](../../../.github/workflows/ci.yml) installs `libsystemd-dev` and `libssl-dev`, not SQLite, libpq, or libargon2 development packages.
