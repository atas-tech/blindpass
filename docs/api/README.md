...
An expired, controller-signed grant is discarded only after the broker verifies its envelope and bindings. The broker persists a stable `grant_rejected` audit event before the node relay advances the inbox cursor; the controller checks the node and grant expiry against its stored grant. Other grant validation failures remain retryable or fail closed.
